...
class CalendarAdapter(context: Context, days: ArrayList<Date>, eventDays:HashSet<Date>, inputMonth: Int) :
    ArrayAdapter<Date>(context, R.layout.fragment_main, days) {
    ...
    override fun getView(position: Int, view: View?, parent: ViewGroup): View {
        ...
        if (view == null) { view = inflater.inflate(R.layout.item_day, parent, false) }
     ...
     if (month != inputMonth || year != calendarToday.get(Calendar.YEAR)) {
        ...
        { // 오늘의 날짜에 하고싶은 짓(?)을 정의
            view.setTextColor(Color.parseColor("#ff0ff"))
        }
        // 날짜를 텍스트뷰에 설정
